...
<?php if (findAllPostByUser($user['id'])): ?>
	<a href="create.php" class="btn btn-primary float-right" style="margin: 5px">Novo Post</a>
	...
	<?php foreach (findAllPostByUser($user['id']) as $post):?>
		<tr>
			<td><?= $post['id']?></td>
			<td><?= $post['titulo']?></td>
			<td><?= ($post['publicado'] == 't') ? 'Publicado' : 'Náo Publicado'?></td>
...
<?php else: ?>
	...
	<p><a href="create.php" class="btn btn-primary" style="margin: 5px">Novo Post</a></p>
<?php endif; ?>
